...
def env_config() -> EnvironmentConfig:
    ...
    config.fee = 0.005
    config.slippage_atr_based = 0.01
    config.slippage_against_trade_probability = 0.6
    config.start_balance = 10000.0
    config.max_daily_drawdown = 0.02
    config.max_alltime_drawdown = 0.05
    config.max_percentage_open_position = 100.0
    config.min_percentage_open_position = 1.0
    config.max_time_in_trade = 10
    config.optimal_exit_time = 3
    config.variance_penalty_weight = 0.5
    config.atr_penalty_weight = 0.3
    return config
