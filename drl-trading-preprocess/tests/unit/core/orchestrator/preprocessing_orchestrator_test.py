...
class TestPreprocessingOrchestratorSuccessFlow:
    ...
    def test_successful_processing_publishes_completion_notification(
        ...
        mock_dependencies["feature_computer"].compute_batch.return_value = (
            sample_features_df
        )
        ...
        mock_dependencies[
            "feature_coverage_analyzer"
        ].analyze_feature_coverage.return_value = coverage_analysis

        # Configure coverage evaluator to return features needing computation
        mock_dependencies[
            "feature_coverage_evaluator"
        ].get_features_needing_computation.return_value = ["test_feature"]
        mock_dependencies[
            "feature_coverage_evaluator"
        ].get_features_needing_warmup.return_value = []
        ...
        mock_dependencies[
            "message_publisher"
        ].publish_preprocessing_completed.assert_called_once()

        # Verify call arguments
        call_args = mock_dependencies[
            "message_publisher"
        ].publish_preprocessing_completed.call_args
    ...
    def test_multiple_timeframes_publishes_correct_metrics(
        ...
        mock_dependencies[
            "feature_coverage_analyzer"
        ].analyze_feature_coverage.side_effect = mock_analyze_coverage
        ...
        assert call_args.kwargs["processing_context"] == "training"
        assert (
            call_args.kwargs["total_features_computed"] == 4
        )  # 2 features × 2 timeframes
...
class TestPreprocessingOrchestratorErrorHandling:
    ...
    def test_validation_errors_publish_failure_notification(
        ...
        mock_dependencies[
            "message_publisher"
        ].publish_feature_validation_error.assert_called_once()

        # Verify error details contain validation info
        call_args = mock_dependencies[
            "message_publisher"
        ].publish_feature_validation_error.call_args
        assert call_args.kwargs["request"] == request
    ...
    def test_storage_failure_publishes_failure_notification(
        ...
        mock_dependencies[
            "feature_store_port"
        ].store_computed_features_offline.side_effect = RuntimeError(
        ...
        mock_dependencies[
            "message_publisher"
        ].publish_preprocessing_error.assert_called_once()

        # Verify error details contain storage info
        call_args = mock_dependencies[
            "message_publisher"
        ].publish_preprocessing_error.call_args
        assert call_args.kwargs["request"] == request
...
class TestPreprocessingOrchestratorSkipExisting:
    ...
    def test_all_features_fully_covered_skips_computation(
        ...
        request = core_request_factory(skip_existing_features=True)
        ...
        existing_features_df.index = pd.date_range(
            "2023-01-01", periods=3, freq="5min", tz=timezone.utc
        )
        ...
        mock_dependencies[
            "feature_coverage_analyzer"
        ].analyze_feature_coverage.return_value = full_coverage
        mock_dependencies["feature_validator"].validate_definitions.return_value = {
            "test_feature": True
        }
        ...
        mock_dependencies[
            "feature_coverage_evaluator"
        ].get_features_needing_computation.return_value = []
        ...
        mock_dependencies[
            "market_data_resampler"
        ].resample_symbol_data_incremental.assert_not_called()
        mock_dependencies["feature_computer"].compute_batch.assert_not_called()
        mock_dependencies[
            "feature_store_port"
        ].store_computed_features_offline.assert_not_called()
    ...
    def test_no_ohlcv_data_forces_resampling_and_computation(
        ...
        mock_dependencies[
            "feature_coverage_analyzer"
        ].analyze_feature_coverage.return_value = no_ohlcv_coverage
        mock_dependencies["feature_validator"].validate_definitions.return_value = {
            "test_feature": True
        }
        ...
        mock_dependencies[
            "market_data_resampler"
        ].resample_symbol_data_incremental.assert_called_once()
    ...
    def test_warmup_failure_publishes_error_notification(
        ...
        with patch.object(
            preprocessing_orchestrator, "_handle_feature_warmup", return_value=False
        ):
            # When
            result = preprocessing_orchestrator.process_feature_computation_request(
                request
            )
            ...
            mock_dependencies[
                "message_publisher"
            ].publish_preprocessing_completed.assert_not_called()
...
class TestPreprocessingOrchestratorEdgeCases:
    ...
    def test_partial_timeframe_failure_still_publishes_successful_timeframes(
        ...
        mock_dependencies[
            "feature_coverage_analyzer"
        ].analyze_feature_coverage.side_effect = [coverage_5m, coverage_15m]
    ...
    def test_request_with_large_feature_set_handles_correctly(
        ...
        # Given
        # Create 100 feature names
        large_feature_names = [f"feature_{i}" for i in range(100)]

        request = core_request_factory(
            skip_existing_features=False, feature_names=large_feature_names
        )
        ...
        large_df = pd.DataFrame({f"feature_{i}": [1.0, 2.0, 3.0] for i in range(100)})
    ...
    def test_concurrent_timeframe_processing_with_parallel_enabled(
        ...
        mock_dependencies[
            "feature_coverage_analyzer"
        ].analyze_feature_coverage.side_effect = [
            coverage_5m,
            coverage_15m,
            coverage_30m,
        ]
