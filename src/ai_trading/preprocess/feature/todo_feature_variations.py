...
class FeatureEngineer:
    ...
    def compute_roc(self, column: str, periods: list) -> pd.DataFrame:
        ...
        for period in periods:
            self.df[f"roc_{column}_{period}"] = np.log(
                self.df[column] / self.df[column].shift(period)
            )
        return self.df
    ...
    def compute_volatility_agnostic_distance(
        self, indicator: str, atr_length: int
    ) -> pd.Series:
        """Normalize price-based indicators using ATR."""
        atr = talib.ATR(
            self.df["High"], self.df["Low"], self.df["Close"], timeperiod=atr_length
        )
    ...
    def compute_percentile_rank(self, column: str, window: int) -> pd.Series:
        ...
        return (
            self.df[column]
            .rolling(window)
            .apply(lambda x: np.argsort(x)[-1] / len(x), raw=True)
        )
