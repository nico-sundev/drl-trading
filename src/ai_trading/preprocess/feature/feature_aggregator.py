...
class FeatureAggregator:
    ...
    def _get_historical_features(self, feature_name: str, param_set) -> Optional[DataFrame]:
        ...
        try:
            ...
            entity_df[self.feature_store_config.entity_name] = self._get_symbol_from_df()
            
            # Get feature view name based on feature and params
            feature_view_name = f"{feature_name}_{param_set.hash_id()}"
            feature_refs = [f"{feature_view_name}:{col}" for col in self.class_registry.feature_class_map[feature_name]([]).get_sub_features_names(param_set)]
    ...
    def compute(self) -> DataFrame:
        ...
        for feature in self.config.feature_definitions:
            ...
            for param_set in feature.parsed_parameter_sets:
                ...
                historical_features = self._get_historical_features(feature.name, param_set)
                if historical_features is not None:
                    feature_results.append(historical_features)
                    continue
                
                # Compute features if not found in store
                logger.info(f"Computing features for {feature.name} with params {param_set}")
        ...
        return concat([df.set_index("Time") for df in feature_results], axis=1).reset_index()
        
    def _store_computed_features(self, feature_df: DataFrame, feature_name: str, param_set) -> None:
        ...
        try:
            ...
            feature_df[self.feature_store_config.entity_name] = self._get_symbol_from_df()
            
            feature_view_name = f"{feature_name}_{param_set.hash_id()}"
            logger.info(f"Storing computed features in feature store: {feature_view_name}")
            ...
            try:
                ...
                logger.info(f"Feature view {feature_view_name} does not exist, creating...")
