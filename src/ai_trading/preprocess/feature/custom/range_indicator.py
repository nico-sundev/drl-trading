...
class SupportResistanceFinder:
    ...
    def find_pivot_points(self, origin: pd.DataFrame) -> pd.DataFrame:
        """
        Identifies pivot highs and lows (local maxima/minima) in the dataset.
        """
        df = origin.copy()  # Avoid modifying original DataFrame
        df["pivot_high"] = (df["Close"].shift(1) < df["Open"].shift(1)) & (
            df["Close"].shift(2) > df["Open"].shift(2)
        )
        df["pivot_low"] = (df["Close"].shift(1) > df["Open"].shift(1)) & (
            df["Close"].shift(2) < df["Open"].shift(2)
        )
        return df
    ...
    def find_next_support_resistance(self, df: pd.DataFrame, last_close: float):
        ...
        """
        support_zone, resistance_zone = {"low": np.nan, "high": np.nan}, {
            "low": np.nan,
            "high": np.nan,
        }
        ...
        for i in range(
            len(df) - 2, max(-1, len(df) - 2 - self.config.range.lookback), -1
        ):
            row, prev_row = df.iloc[i], df.iloc[i - 1] if i > 0 else None

            if (
                not found_resistance
                and row["pivot_high"]
                and prev_row is not None
                and prev_row["Close"] < row["Close"]
                and row["Close"] > last_close
            ):
                found_resistance = True
                resistance_zone["low"] = prev_row["Close"]
                resistance_zone["high"] = self.calculate_wick_threshold(
                    row["Close"], max(row["High"], prev_row["High"])
                )
            ...
            if (
                ...
                support_zone["low"] = self.calculate_wick_threshold(
                    row["Close"], min(row["Low"], prev_row["Low"])
                )
    ...
    def calculate_wick_threshold(
        close: float, extreme: float, ratio: float = 2 / 3
    ) -> float:
        """
    ...
    def find_support_resistance_zones(self, df: pd.DataFrame) -> pd.DataFrame:
        ...
        resistance_valid = (
            self.prev_resistance and last_close < self.prev_resistance["high"]
        )
        ...
        support_zone, resistance_zone = self.find_next_support_resistance(
            df, last_close
        )

        # Cache the results
        self.prev_support = support_zone if not support_valid else self.prev_support
        self.prev_resistance = (
            resistance_zone if not resistance_valid else self.prev_resistance
        )
