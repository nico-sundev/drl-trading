...
def test_strip_higher_timeframes(base_dataset, higher_timeframe_dataset):
    ...
    base_end_timestamp = base_dataset["Time"].iloc[-1]  # Last timestamp of the base dataset

    # Act
    stripped_df = service.strip_higher_timeframes(base_end_timestamp, higher_timeframe_dataset)

    # Assert
    assert not stripped_df.empty, "The stripped dataframe should not be empty."
    assert stripped_df["Time"].iloc[-1] < base_end_timestamp + timedelta(hours=8), (
...
def test_strip_asset_price_datasets(base_dataset, higher_timeframe_dataset):
    ...
    assert stripped_base_dataset.timeframe == "H1", "The base dataset should retain its timeframe."
    assert stripped_higher_timeframe_dataset.timeframe == "H4", "The higher timeframe dataset should retain its timeframe."

    # Ensure the higher timeframe dataset is stripped correctly
    base_end_timestamp = base_dataset["Time"].iloc[-1]
    assert stripped_higher_timeframe_dataset.asset_price_dataset["Time"].iloc[-1] < base_end_timestamp + timedelta(hours=8), (
