...
def mock_finder():
    ...
    return SupportResistanceFinder(lookback=5)
...
def test_validate_dataframe_nan_values(mock_finder):
    ...
    df = pd.DataFrame({"Close": [100, np.nan, 102], "High": [101, 103, 102], "Low": [99, 100, 100]})
...
def test_zone_reset_when_broken(mock_finder, mock_data):
    ...
    assert support == {"low": 102, "high": 104}
    assert resistance == {"low": np.nan, "high": np.nan}  # Reset since resistance was broken

def test_calculate_wick_threshold():
    """Test that the wick threshold calculation is correct."""
    close, extreme = 100, 110
    assert SupportResistanceFinder.calculate_wick_threshold(close, extreme, ratio=0.5) == 105
    ...
    assert lower_bound <= result <= upper_bound, f"Expected {expected_value} ± {tolerance_percentage*100}%, but got {result}"
...
def test_find_support_resistance_zones(mock_finder, mock_data):
    ...
    assert not np.isnan(last_row["support_zone_low"]) or not np.isnan(last_row["resistance_zone_high"])
