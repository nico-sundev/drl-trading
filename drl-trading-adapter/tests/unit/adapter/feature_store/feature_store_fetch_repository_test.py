...
class TestFeatureStoreFetchRepositoryGetOnline:
    ...
    def test_get_online_raises_error_when_feature_service_is_none(
        ...
        # Given
        mock_feast_provider.get_feature_service.side_effect = RuntimeError("Feature service not found")

        # When & Then
        with pytest.raises(RuntimeError, match="Feature service not found"):
