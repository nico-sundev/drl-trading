...
def configure_logging():
    ...
    loggers = ["drl_trading_adapter", "drl_trading_common", "feast", "root"]
...
def temp_feast_repo() -> Generator[str, None, None]:
    ...
    import subprocess
    try:
        subprocess.run(["feast", "init", temp_dir], check=True, capture_output=True)
    except (subprocess.CalledProcessError, FileNotFoundError):
        # If feast not available, create minimal feature_store.yaml
        import yaml
        feature_store_yaml = os.path.join(temp_dir, "feature_store.yaml")
        with open(feature_store_yaml, 'w') as f:
            yaml.dump({
                "project": "feast_integration_test",
