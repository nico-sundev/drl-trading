...
def preprocess_market_data(
    df, pattern_length=7, prediction_length=7, similarity_threshold=0.8
):
    patterns = []
    targets = []
    indices = []
    ...
    for i in range(len(df) - pattern_length - prediction_length):
        ...
        pattern = df.iloc[i : i + pattern_length][
            ["Open", "High", "Low", "Close"]
        ].values.flatten()
    ...
    y = torch.tensor(np.array(targets), dtype=torch.float).unsqueeze(
        1
...
def run_example(df):
    """Run the GNN example with the provided dataframe."""
    # Apply function
    ...
    return query_gnn(gnn_model, current_market_state, market_graph)
