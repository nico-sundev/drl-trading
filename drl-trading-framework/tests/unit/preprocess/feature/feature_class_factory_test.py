...
def test_reset(factory: FeatureClassFactory) -> None:
    """Test factory reset functionality."""
    # Given
    factory.register_feature_class("macd", MacdFeature)
    assert factory.get_feature_class("macd") is not None

    # When
    factory.reset()

    # Then
    assert factory.get_feature_class("macd") is None
    assert len(factory._feature_class_map) == 0
