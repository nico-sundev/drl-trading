...
@pytest.fixture
def dataset(mocked_container: Injector) -> AssetPriceDataSet:
...
@pytest.fixture
def expected_features(mocked_container: Injector) -> Dict[str, Set[str]]:
    ...
    application_config = mocked_container.get(ApplicationConfig)
    features_config = mocked_container.get(FeaturesConfig)
    ...
    for feature_def in features_config.feature_definitions:
        ...
        for param_set in feature_def.parsed_parameter_sets:
            if not param_set.enabled:
                continue            # Add expected column name patterns based on feature class implementations
            if feature_name == "rsi":
                # From RsiFeature.get_sub_features_names()
                # The parameter set should be an RsiConfig for rsi features
                if hasattr(param_set, 'length'):
                    expected[feature_name].add(f"rsi_{param_set.length}")

    return expected


def test_feature_computation_and_caching(
    mocked_container: Injector, dataset, expected_features
):
    ...
    for feature_name, expected_cols in expected_features.items():
        for col_substring in expected_cols:
            ...
            assert (
                len(matching_cols) > 0
            ), f"Feature {feature_name} with substring '{col_substring}' not found in first run"    # 3. Verify second run produced identical results to first run
